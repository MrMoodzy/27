...
function draw() {
  ...
  for (var i = 0; i < playerArrows.length; i++) {
    if (playerArrows[i] !== undefined) {
      ...
      d1 = dist(PlayerArrow[i].body.position.x,PlayerArrow[i].body.position.y,board1.body.position.x, board1.body.position.y)

      if(d1<=100){
        console.log("collided");
      }
      varboard1Collision = Matter.SAT.collides(
        board1.body,
        playerArrows[i].body
      );
      
      varboard2Collision = Matter.SAT.collides(
        board2.body,
        playerArrows[i].body
      );
